...
typedef struct uac_host_device {
    ...
    bool newo_ctrl_inflight;                       /*!< Protected by UAC lock */
...
typedef struct uac_interface {
    ...
    newo_uac_stats_t stats;                    /*!< Newo bounded diagnostics */
...
static esp_err_t uac_host_interface_add(uac_device_t *uac_device, uint8_t iface_num, uac_iface_t **p_uac_iface)
{
    ...
    *p_uac_iface = NULL;
    ...
    UAC_GOTO_ON_FALSE(iface_desc, ESP_ERR_NOT_SUPPORTED, "Missing alternate zero");
    ...
    while (iface_alt_desc != NULL) {
        ...
        iface_alt_idx++;
        // Preserve the original allocation on realloc failure.
        void *resized = realloc(uac_iface->iface_alt, iface_alt_idx * sizeof(uac_iface_alt_t));
        UAC_GOTO_ON_FALSE(resized, ESP_ERR_NO_MEM, "Unable to allocate memory");
        uac_iface->iface_alt = resized;
        ...
        while (cs_desc != NULL && parse_continue && cs_desc->bDescriptorType != USB_B_DESCRIPTOR_TYPE_INTERFACE) {
            ...
            case UAC_CS_INTERFACE: {
                ...
                if (uac_desc->bDescriptorSubtype == UAC_AS_GENERAL) {
                    ...
                    iface_alt->dev_alt_param.sample_freq_type = MIN(as_format_type_desc->bSamFreqType, UAC_FREQ_NUM_MAX);
            ...
            case USB_B_DESCRIPTOR_TYPE_ENDPOINT: {
                ...
                // Never alter a device-owned endpoint descriptor to make it fit.
                UAC_GOTO_ON_FALSE(dev_info_local.speed == USB_SPEED_FULL && ep_desc->bInterval == 1,
                                  ESP_ERR_NOT_SUPPORTED, "Requires full-speed bInterval=1");
                ...
                if (feature_unit_desc && feature_unit_desc->bLength >= 7 && feature_unit_desc->bControlSize) {
    ...
    UAC_GOTO_ON_FALSE(iface_alt_idx > 0, ESP_ERR_NOT_SUPPORTED, "No streaming alternate");
...
static esp_err_t uac_host_interface_check(uint8_t addr, const usb_config_desc_t *config_desc)
{
    ...
    while (iface_desc != NULL) {
        if (iface_desc->bInterfaceClass == USB_CLASS_AUDIO && iface_desc->bInterfaceSubClass == UAC_SUBCLASS_AUDIOSTREAMING) {
            ...
            const usb_ep_desc_t *ep_desc = (iface_alt_desc && iface_alt_desc->bInterfaceNumber == iface_desc->bInterfaceNumber)
                ? usb_parse_endpoint_descriptor_by_index(iface_alt_desc, 0, total_length, &ep_offset) : NULL;
            if (ep_desc == NULL) {
...
static esp_err_t _uac_host_device_connected(uint8_t addr)
{
    ...
    if (is_uac_device) {
        ...
        uac_host_interface_check(addr, config_desc);
    } else {
...
static esp_err_t _uac_host_device_disconnected(usb_device_handle_t dev_hdl)
{
    ...
    if (!uac_device) return ESP_OK;

    // Newo: never close synchronously from the USB event task. Closing can
    // wait for control/completion callbacks on this same task. The application
    // callback only marks removal; its worker closes while events keep running.
    const uint8_t address = uac_device->addr;
    UAC_ENTER_CRITICAL();
    uac_iface_t *uac_iface = STAILQ_FIRST(&s_uac_driver->uac_ifaces_tailq);
    while (uac_iface) {
        uac_iface_t *next_iface = STAILQ_NEXT(uac_iface, tailq_entry);
        // Newo callback is atomic flags only. Keep the list stable until
        // every matching interface has its disconnect flag set.
        if (uac_iface->dev_info.addr == address)
            uac_host_user_interface_callback(uac_iface, UAC_HOST_DRIVER_EVENT_DISCONNECTED);
...
static esp_err_t uac_host_interface_claim_and_prepare_transfer(uac_iface_t *iface)
{
    ...
    UAC_RETURN_ON_ERROR(usb_host_interface_claim(s_uac_driver->client_handle, iface->parent->dev_hdl, iface->dev_info.iface_num,
                                                 iface->iface_alt[iface->cur_alt].alt_idx), "Unable to claim Interface");
...
static void stream_rx_xfer_done(usb_transfer_t *in_xfer)
{
    ...
    case USB_TRANSFER_STATUS_COMPLETED: {
        ...
        if (data_len + in_xfer->actual_num_bytes > iface->ringbuf_size) {
            UAC_ENTER_CRITICAL();
            iface->stats.dropped_packets += in_xfer->num_isoc_packets;
            UAC_EXIT_CRITICAL();
            ...
            for (int i = 0; i < in_xfer->num_isoc_packets; i++) {
                ...
                // copy data to ringbuffer
                esp_err_t push = _ring_buffer_push(iface->ringbuf, in_xfer->data_buffer + i * requested_num_bytes, actual_num_bytes, 0);
                UAC_ENTER_CRITICAL();
                iface->stats.packets++;
                iface->stats.bytes += actual_num_bytes;
                if (push != ESP_OK) iface->stats.dropped_packets++;
                UAC_EXIT_CRITICAL();
            }
        }
        // Relaunch transfer
        if (usb_host_transfer_submit(in_xfer) != ESP_OK) {
            UAC_ENTER_CRITICAL(); iface->stats.submit_errors++; UAC_EXIT_CRITICAL();
        ...
        data_len = _ring_buffer_get_len(iface->ringbuf);
        UAC_ENTER_CRITICAL();
        if (data_len > iface->stats.buffer_high_water) iface->stats.buffer_high_water = data_len;
        UAC_EXIT_CRITICAL();
...
static void stream_tx_xfer_submit(usb_transfer_t *out_xfer)
{
    ...
    if (data_len >= urb_size) {
        ...
        if (usb_host_transfer_submit(out_xfer) != ESP_OK) {
            UAC_ENTER_CRITICAL(); iface->stats.submit_errors++; UAC_EXIT_CRITICAL();
            uac_host_user_interface_callback(iface, UAC_HOST_DEVICE_EVENT_TRANSFER_ERROR);
        }
        data_len = _ring_buffer_get_len(iface->ringbuf);
...
static void stream_tx_xfer_done(usb_transfer_t *out_xfer)
{
    ...
    case USB_TRANSFER_STATUS_COMPLETED: {
        UAC_ENTER_CRITICAL();
        for (int i = 0; i < out_xfer->num_isoc_packets; ++i) {
            if (out_xfer->isoc_packet_desc[i].status == USB_TRANSFER_STATUS_COMPLETED) {
                iface->stats.packets++;
                iface->stats.bytes += out_xfer->isoc_packet_desc[i].actual_num_bytes;
            } else iface->stats.packet_errors++;
        }
        UAC_EXIT_CRITICAL();
...
static esp_err_t uac_host_interface_suspend(uac_iface_t *iface)
{
    ...
    ret = usb_host_endpoint_halt(iface->parent->dev_hdl, ep_addr);
    if (ret == ESP_OK) ret = usb_host_endpoint_flush(iface->parent->dev_hdl, ep_addr);
    if (ret != ESP_OK) {
        iface->state = UAC_INTERFACE_STATE_ACTIVE; // Keep ownership; caller retries.
        return ret;
    }
...
static esp_err_t uac_host_interface_resume(uac_iface_t *iface)
{
    ...
    usb_setup_packet_t usb_request;
    USB_SETUP_PACKET_INIT_SET_INTERFACE(&usb_request, iface->dev_info.iface_num, iface->iface_alt[iface->cur_alt].alt_idx);

    // memcpy, because of alignment warning
    uac_cs_request_t uac_request = {0};
    memcpy(&uac_request, &usb_request, sizeof(usb_setup_packet_t));
    UAC_RETURN_ON_ERROR(uac_cs_request_set(iface->parent, &uac_request), "Unable to set Interface alternate");
    ESP_LOGI(TAG, "Resume Interface %d-%d", iface->dev_info.iface_num, iface->iface_alt[iface->cur_alt].alt_idx);
    ...
    // Mark ACTIVE before submitting: a completion may run immediately.
    ...
    while (iface_desc != NULL) {
        ...
        if (iface_desc->bInterfaceClass == USB_CLASS_AUDIO && iface_desc->bInterfaceSubClass == UAC_SUBCLASS_AUDIOCONTROL) {
            ...
            while (uac_cs_desc != NULL) {
                ...
                uac_cs_desc = (uac_desc_header_t *)GET_NEXT_DESC(uac_cs_desc, total_length, uac_desc_offset);
                if (!uac_cs_desc || uac_cs_desc->bDescriptorType != UAC_CS_INTERFACE) {
...
fail:
    if (uac_device) {
        if (uac_device->ctrl_xfer) usb_host_transfer_free(uac_device->ctrl_xfer);
        if (uac_device->ctrl_xfer_done) vSemaphoreDelete(uac_device->ctrl_xfer_done);
        if (uac_device->device_busy) vSemaphoreDelete(uac_device->device_busy);
...
static void ctrl_xfer_done(usb_transfer_t *ctrl_xfer)
{
    ...
    UAC_ENTER_CRITICAL();
    uac_device->newo_ctrl_inflight = false;
    UAC_EXIT_CRITICAL();
...
static esp_err_t uac_control_transfer(uac_device_t *uac_device, int len, uint32_t timeout_ms)
{
    ...
    UAC_ENTER_CRITICAL();
    bool pending = uac_device->newo_ctrl_inflight;
    UAC_EXIT_CRITICAL();
    UAC_RETURN_ON_FALSE(!pending, ESP_ERR_INVALID_STATE, "Previous control completion pending");
    xSemaphoreTake(uac_device->ctrl_xfer_done, 0);
    ...
    UAC_ENTER_CRITICAL();
    uac_device->newo_ctrl_inflight = true;
    UAC_EXIT_CRITICAL();
    esp_err_t submit = usb_host_transfer_submit_control(s_uac_driver->client_handle, ctrl_xfer);
    if (submit != ESP_OK) {
        UAC_ENTER_CRITICAL(); uac_device->newo_ctrl_inflight = false; UAC_EXIT_CRITICAL();
        return submit;
    }
    ...
    if (received != pdTRUE) {
        ...
        // Completion must return ownership before reuse/free. If still pending,
        // next control request/close refuses rather than touching an in-flight URB.
        xSemaphoreTake(uac_device->ctrl_xfer_done, pdMS_TO_TICKS(timeout_ms));
        return ESP_ERR_TIMEOUT;
    }
...
fail:
    if (uac_iface) {
        if (uac_iface->ringbuf) vRingbufferDelete(uac_iface->ringbuf);
...
esp_err_t uac_host_device_close(uac_host_device_handle_t uac_dev_handle)
{
    ...
    UAC_ENTER_CRITICAL();
    if (uac_iface->parent->opened_cnt == 1) {
        bool pending = uac_iface->parent->newo_ctrl_inflight;
        UAC_EXIT_CRITICAL();
        UAC_GOTO_ON_FALSE(!pending, ESP_ERR_INVALID_STATE, "Control completion pending");
...
fail:
    if (iface_claimed) {
        // Partial submission must drain before freeing transfers. Close retries
        // safely if the host still has completions pending.
        if (iface->state == UAC_INTERFACE_STATE_ACTIVE) uac_host_interface_suspend(iface);
        if (iface->state == UAC_INTERFACE_STATE_READY) uac_host_interface_release_and_free_transfer(iface);
    }
    uac_host_interface_unlock(iface);
    return ret;
}
...
esp_err_t uac_host_device_write(uac_host_device_handle_t uac_dev_handle, uint8_t *data, uint32_t size, uint32_t timeout)
{
    ...
    size_t newo_len = _ring_buffer_get_len(iface->ringbuf);
    UAC_ENTER_CRITICAL();
    if (newo_len > iface->stats.buffer_high_water) iface->stats.buffer_high_water = newo_len;
    UAC_EXIT_CRITICAL();
...
esp_err_t uac_host_device_set_mute(uac_host_device_handle_t uac_dev_handle, bool mute)
{
    ...
    ESP_LOGI(TAG, "%s Interface %d-%d", mute ? "Mute" : "Unmute", iface->dev_info.iface_num, iface->iface_alt[iface->cur_alt].alt_idx);
    uac_host_interface_unlock(iface);
    return ESP_OK;
...
esp_err_t uac_host_device_set_volume(uac_host_device_handle_t uac_dev_handle, uint8_t volume)
{
    ...
    ESP_LOGI(TAG, "Set volume %d%%, Interface %d-%d", volume, iface->dev_info.iface_num, iface->iface_alt[iface->cur_alt].alt_idx);
    uac_host_interface_unlock(iface);
    return ESP_OK;
...
// Newo: called only while the application still owns the interface handle.
esp_err_t newo_uac_get_stats(uac_host_device_handle_t handle, newo_uac_stats_t *stats)
{
    uac_iface_t *iface = get_iface_by_handle(handle);
    if (!iface || !stats) return ESP_ERR_INVALID_ARG;
    UAC_ENTER_CRITICAL();
    *stats = iface->stats;
    UAC_EXIT_CRITICAL();
    return ESP_OK;
