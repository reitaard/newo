...
// Newo diagnostic extension: cumulative counters, reset on every start.
// Packet errors include missed/late isochronous packets; USB does not retry them.
typedef struct {
    uint32_t bytes, packets, packet_errors, dropped_packets;
    uint32_t underruns, submit_errors, buffer_high_water;
} newo_uac_stats_t;
