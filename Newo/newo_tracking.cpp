...
bool NewoTracking::start(){
  ...
  if(esp_wifi_set_csi_config(&cfg)!=ESP_OK||esp_wifi_set_csi_rx_cb(newoCsiRxCallback,nullptr)!=ESP_OK||esp_wifi_set_csi(true)!=ESP_OK){newoCsiMeasurementStopAccepting();newoCollectorDiscoveryStop();peerStatus_=requestPeer(false)?"stopped":"uncertain";endPeer();lastError_="csi_start_failed";return false;}
  senderStop=senderExited=false;if(xTaskCreatePinnedToCore(sender,"newo_track",6144,nullptr,3,&senderTask,0)!=pdPASS){esp_wifi_set_csi(false);esp_wifi_set_csi_rx_cb(nullptr,nullptr);newoCsiMeasurementStopAccepting();newoCollectorDiscoveryStop();peerStatus_=requestPeer(false)?"stopped":"uncertain";endPeer();lastError_="sender_start_failed";return false;}
  peerMonitorStop=peerMonitorExited=false;if(xTaskCreatePinnedToCore(peerMonitorEntry,"newo_peer",3072,this,2,&peerMonitorTask,0)!=pdPASS){esp_wifi_set_csi(false);esp_wifi_set_csi_rx_cb(nullptr,nullptr);newoCsiMeasurementStopAccepting();senderStop=true;uint32_t began=millis();while(!senderExited&&millis()-began<1000)vTaskDelay(pdMS_TO_TICKS(10));if(!senderExited){newoCsiMeasurementDiscardPending();if(senderTask){vTaskDelete(senderTask);senderTask=nullptr;}senderExited=true;}newoCollectorDiscoveryStop();peerStatus_=requestPeer(false)?"stopped":"uncertain";endPeer();lastError_="peer_monitor_start_failed";return false;}
