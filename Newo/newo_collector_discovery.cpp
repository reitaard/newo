...
uint32_t fallbackAddress, discoveredAddress; uint16_t fallbackPort, discoveredPort;
...
NewoCollectorDestination newoCollectorSnapshot(){uint32_t a;uint16_t p;bool fresh;portENTER_CRITICAL(&mux);fresh=discoveredAddress&&esp_timer_get_time()<leaseDeadline;a=fresh?discoveredAddress:fallbackAddress;p=fresh?discoveredPort:fallbackPort;portEXIT_CRITICAL(&mux);return{a,p,fresh?"discovered":"configured"};}
