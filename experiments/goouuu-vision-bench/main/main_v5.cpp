...
// The camera owns its DMA framebuffers. We copy each published JPEG exactly once
// into this small pool, then the stream and AI tasks borrow the same bytes by
// reference. A referenced slot is never overwritten.
struct JpegSlot {
...
bool open_decoder(DecoderContext &ctx,
    ...
    ctx.config.block_enable = block_mode;
    if (!block_mode && scale_w && scale_h) {
    ...
    ESP_LOGI(TAG, "jpeg decoder ready name=%s scale=%ux%u block=%d",
             name, static_cast<unsigned>(scale_w), static_cast<unsigned>(scale_h), block_mode ? 1 : 0);
    return true;
}
...
bool decode_with_context(DecoderContext &ctx,
                         ...
                         uint16_t &out_h,
                         uint32_t &decode_ms) {
    if (!ctx.handle || !jpeg.data || jpeg.len == 0 || ctx.config.block_enable) return false;
...
// esp_new_jpeg's normal "clipper" only crops from the top-left; it has no x/y
// offset. RANGE therefore uses block decoding. We decode sequential MCU rows and
// copy only the selected x/y window into the detector tile. This keeps arbitrary
// four-corner ROIs without ever materializing a full 640x480 RGB565 frame.
bool extract_range_tile(DecoderContext &ctx,
                        const JpegLease &jpeg,
                        int tile_index,
                        uint8_t *&tile,
                        ...
                        uint32_t &decode_ms,
                        uint32_t &prep_ms) {
    if (!ctx.handle || !ctx.config.block_enable || !jpeg.data || jpeg.len == 0) return false;
    ...
    if (jpeg_dec_parse_header(ctx.handle, &io, &info) != JPEG_ERR_OK) return false;
    decode_us += esp_timer_get_time() - t;

    if (info.width == 0 || info.height == 0) return false;
    tile_w = (static_cast<int>(info.width) * 3) / 5;
    tile_h = (static_cast<int>(info.height) * 3) / 5;
    // VGA makes these 384x288. Keep multiples of 8 for detector-friendly ROIs.
    tile_w &= ~7;
    tile_h &= ~7;
    if (tile_w <= 0 || tile_h <= 0) return false;

    const int xs[2] = {0, static_cast<int>(info.width) - tile_w};
    const int ys[2] = {0, static_cast<int>(info.height) - tile_h};
    const int xi = tile_index & 1;
    const int yi = (tile_index >> 1) & 1;
    x0 = xs[xi];
    y0 = ys[yi];

    const size_t tile_bytes = static_cast<size_t>(tile_w) * tile_h * 2;
    if (!ensure_psram_buffer(tile, tile_capacity, tile_bytes, true)) return false;

    int block_len = 0;
    int process_count = 0;
    if (jpeg_dec_get_outbuf_len(ctx.handle, &block_len) != JPEG_ERR_OK || block_len <= 0) return false;
    if (jpeg_dec_get_process_count(ctx.handle, &process_count) != JPEG_ERR_OK || process_count <= 0) return false;
    if (!ensure_psram_buffer(block, block_capacity, static_cast<size_t>(block_len), true)) return false;

    io.outbuf = block;
    const size_t source_row_bytes = static_cast<size_t>(info.width) * 2;
    const size_t tile_row_bytes = static_cast<size_t>(tile_w) * 2;
    int current_y = 0;
    int copied_rows = 0;
    ...
    for (int block_index = 0; block_index < process_count; ++block_index) {
        ...
        if (io.out_size <= 0 || source_row_bytes == 0 || (static_cast<size_t>(io.out_size) % source_row_bytes) != 0) {
            return false;
        }

        const int block_rows = static_cast<int>(static_cast<size_t>(io.out_size) / source_row_bytes);
        const int block_y0 = current_y;
        const int block_y1 = current_y + block_rows;
        const int copy_y0 = std::max(y0, block_y0);
        const int copy_y1 = std::min(y0 + tile_h, block_y1);

        if (copy_y1 > copy_y0) {
            t = esp_timer_get_time();
            for (int y = copy_y0; y < copy_y1; ++y) {
                const int source_row = y - block_y0;
                const int tile_row = y - y0;
                const uint8_t *src = block + static_cast<size_t>(source_row) * source_row_bytes + static_cast<size_t>(x0) * 2;
                uint8_t *dst = tile + static_cast<size_t>(tile_row) * tile_row_bytes;
                memcpy(dst, src, tile_row_bytes);
                ++copied_rows;
            }
            prep_us += esp_timer_get_time() - t;
        }
        current_y = block_y1;
    }

    decode_ms = static_cast<uint32_t>((decode_us + 500) / 1000);
    prep_ms = static_cast<uint32_t>((prep_us + 500) / 1000);
    return copied_rows == tile_h;
}
    ...
    for (;;) {
        ...
        if (fb->format == PIXFORMAT_JPEG) {
            ...
            if (slot_index >= 0) {
                ...
                if (ensure_psram_buffer(slot.data, slot.capacity, fb->len)) {
                    memcpy(slot.data, fb->buf, fb->len);
                    publish_copy_ms = static_cast<uint32_t>((esp_timer_get_time() - copy_started + 500) / 1000);
                    publish_slot(slot_index, fb->len, fb->width, fb->height);
                    published = true;
                } else {
                    cancel_publish_slot(slot_index);
                    ESP_LOGE(TAG, "JPEG pool allocation failed len=%u", static_cast<unsigned>(fb->len));
                }
            } else {
                record_pool_drop();
            }
        ...
        previous_ms = now_ms;
        if (published) record_capture(bytes, capture_ms, publish_copy_ms, frame_ms);
        taskYIELD();
    }
}
...
void ai_task(void *) {
    ...
    DecoderContext range_decoder;
    if (!open_decoder(fast_decoder, "fast-320x240", FAST_W, FAST_H, false) ||
        !open_decoder(accurate_decoder, "accurate-vga", 0, 0, false) ||
        !open_decoder(range_decoder, "range-block", 0, 0, true)) {
        ESP_LOGE(TAG, "AI decoder initialization failed");
        close_decoder(fast_decoder);
        close_decoder(accurate_decoder);
        close_decoder(range_decoder);
        vTaskDelete(nullptr);
        return;
    }
    ...
    for (;;) {
        ...
        if (mode == TestMode::RANGE) {
            ...
            int x0 = 0;
            int y0 = 0;
            const bool extracted = extract_range_tile(range_decoder, jpeg, range_tile,
                                                      tile, tile_capacity, block, block_capacity,
                                                      tile_w, tile_h, x0, y0,
                                                      decode_ms, prep_ms);
            release_lease(jpeg);
            if (!extracted) {
                ESP_LOGW(TAG, "RANGE block decode/extract failed tile=%d", range_tile);
                continue;
            }
        ...
        if (!run_detector(detector, decoded, decoded_w, decoded_h,
                          boxes, face_count, source_w, source_h, 0, 0, detect_ms)) {
...
esp_err_t index_handler(httpd_req_t *req) {
    ...
    return httpd_resp_send(req, kIndexHtmlV5, HTTPD_RESP_USE_STRLEN);
}
...
extern "C" void app_main(void) {
    ...
    ESP_LOGI(TAG, "NEWO GOOUUU vision v5: native JPEG + shared leases + persistent decoders + block RANGE");
    ...
    ESP_LOGI(TAG, "READY: v5 native JPEG preview at http://192.168.4.1/ ; stream :81/stream");
}
