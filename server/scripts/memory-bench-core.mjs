...
const modes = String(process.env.BENCH_MODES ?? "FAST,THINK").split(",").map(x => x.trim().toUpperCase()).filter(Boolean);

const MINIMAL_PROMPT = `You extract durable memories for a private voice assistant. Return exactly one JSON object and no markdown: {"memory":[{"text":"atomic fact","type":"preference|fact|relationship|device|location|event|plan","event_at":"ISO-8601 date/time or null"}]}. Extract only facts that are useful in a future conversation: stable preferences, personal facts, relationships, device/location facts, meaningful events, and explicit plans. Do not store the current request, small talk, assistant output, temporary actions, guesses, or uncertain speculation. Split independent facts into separate memories. Preserve the user's language. Empty memory is valid and preferred over junk.`;

const VOICEMEM_STYLE_PROMPT = `You are an additive long-term memory extractor for live voice conversations. Return exactly one JSON object and no markdown with top-level "memory". Each item must contain text, type, event_at, and attribute. Capture durable user/world knowledge as atomic memories without replacing unrelated prior facts. Use type preference|fact|relationship|device|location|event|plan. Resolve explicit relative dates from the supplied reference time when possible; otherwise event_at is null. attribute is a short value-free slot such as "user's residence" or "user's favorite restaurant" for changeable attributes, otherwise an empty string. Preserve the user's language and proper nouns. Do NOT remember audibility/connection checks, identity probes with no new fact, remarks about the conversation, garbled ASR fragments, assistant suggestions or answers, one-off requests, or statements whose only content is that something was asked or said. Do not turn uncertainty (maybe/perhaps/might) into a fact. A useful rule: would this still help a week from now in a different conversation? If not, omit it. Empty memory is valid and common.`;
...
function prompt(style, testCase, referenceNow, mode) {
...
function parseMemories(raw) {
  ...
  const memory = Array.isArray(parsed?.memory) ? parsed.memory : Array.isArray(parsed?.memories) ? parsed.memories : null;
  if (!memory) throw new Error('response missing "memory" array');
  return memory.filter(x => x && typeof x === "object" && String(x.text ?? "").trim()).map(x => ({
    ...x,
    text: String(x.text).trim(),
    type: x.type == null ? null : String(x.type).trim().toLowerCase(),
    event_at: x.event_at ?? null,
  }));
}
    ...
    const req = http.request(url, { method: "POST", headers: { "content-type": "application/json" } }, res => {
      ...
      res.on("end", () => {
        ...
        resolve({ raw, metrics: {
          ...
          total_latency_ms: Math.round(total),
          ...
          tokens_per_s: evalNs && Number.isFinite(done.eval_count) ? Number((done.eval_count / (evalNs / 1e9)).toFixed(2)) : null,
        } });
      });
    });
    req.on("error", reject);
...
function scoreCase(testCase, memories) {
  ...
  for (const wanted of expected) {
    const terms = (wanted.terms ?? []).map(normalize).filter(Boolean);
    let found = -1;
    for (const i of unmatched) {
      const text = normalize(memories[i].text);
      if (terms.every(term => text.includes(term))) { found = i; break; }
    }
    if (found >= 0) {
      unmatched.delete(found);
      const got = memories[found];
      matches.push({ expected: wanted, actual: got,
...
export async function runBenchmark(style) {
    ...
    for (const testCase of cases) {
      ...
      try {
        const generated = await request({
          model,
          prompt: prompt(style, testCase, referenceNow, mode),
          raw: true,
          stream: true,
          keep_alive: -1,
        ...
        rows.push({ id: testCase.id, category: testCase.category, started_at: started,
          input: testCase.input, expected: testCase.expected, output: generated.raw,
          memories, json_valid: parseError == null, parse_error: parseError,
          score: parseError == null ? scoreCase(testCase, memories) : null,
          metrics: generated.metrics });
      } catch (error) {
        rows.push({ id: testCase.id, category: testCase.category, started_at: started, input: testCase.input,
          expected: testCase.expected, output: null, memories: [], json_valid: false,
  ...
  const outputPath = path.join(RESULTS_DIR, `${stamp}-${style}.json`);
  const report = { generated_at: new Date().toISOString(), endpoint, model, seed, max_tokens: maxTokens,
    dataset: dataset.name ?? path.basename(datasetPath), reference_now: referenceNow, style, runs };
