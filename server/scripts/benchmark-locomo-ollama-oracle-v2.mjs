...
const answerMaxTokens = Math.max(32, Number(process.env.LOCOMO_ANSWER_MAX_TOKENS ?? 96));
const seed = Number(process.env.LOCOMO_SEED ?? 42);
const enableThinking = process.env.LOCOMO_THINK === "1";
const categoryIds = String(process.env.LOCOMO_CATEGORIES ?? "4,1,2")
  .split(",").map(x => Number(x.trim())).filter(Number.isFinite);

if (!model) throw new Error("Set LOCOMO_OLLAMA_MODEL to the exact installed Ollama model tag.");
...
const ANSWER_SYSTEM = `Answer the benchmark question using only the supplied evidence. Return exactly one JSON object like {"answer":"..."}. The answer must be the shortest value that fully answers the question. Never repeat the evidence, question, labels, or instructions. Resolve relative time expressions such as yesterday/tomorrow from the timestamp attached to that evidence turn. For a list, use a concise comma-separated list. If the evidence is insufficient, answer "Not mentioned".`;
...
function parseAnswer(content) {
  ...
  if (!cleaned) throw new Error("empty answer content");
  ...
  if (start >= 0 && end > start) {
    try {
      ...
      if (typeof parsed.answer === "string") {
        return { answer: parsed.answer.trim(), json_valid: true };
      }
    } catch {}
  }
  return { answer: cleaned, json_valid: false };
}
...
async function answerQuestion(evidenceText, question) {
  ...
  const body = {
    ...
    options: {
      temperature: 0,
      seed,
      num_predict: enableThinking ? Math.max(answerMaxTokens, 256) : answerMaxTokens,
    },
  };
  ...
  if (!bodyText.trim()) throw new Error("empty Ollama HTTP response");

  let payload;
  try { payload = JSON.parse(bodyText); }
  catch (error) { throw new Error(`invalid Ollama JSON response: ${error.message}; body=${JSON.stringify(bodyText.slice(0, 160))}`); }

  const content = payload?.message?.content;
  const thinking = payload?.message?.thinking;
  if (typeof content !== "string" || !content.trim()) {
    throw new Error(`Ollama returned empty message.content${thinking ? `; thinking=${JSON.stringify(String(thinking).slice(0, 160))}` : ""}`);
  }
...
console.log(`\nLoCoMo generic Ollama oracle v2: model=${model} endpoint=${endpoint}`);
...
console.log(`native template; schema format disabled; think=${enableThinking}; answer_max_tokens=${enableThinking ? Math.max(answerMaxTokens, 256) : answerMaxTokens}\n`);
...
for (let i = 0; i < selected.length; i++) {
  ...
  try {
    ...
    row.thinking = answered.thinking;
    row.json_valid = answered.json_valid;
    row.score = scoreAnswer(answered.answer, row.answer, row.category);
    row.metrics = answered.metrics;
    console.log(`  ollama    F1=${(row.score * 100).toFixed(1)}% answer=${JSON.stringify(row.prediction)} json=${row.json_valid} latency=${row.metrics.total_latency_ms}ms t/s=${row.metrics.tokens_per_s ?? "n/a"}`);
  } catch (error) {
...
const summary = {
  ...
  json_valid_rate: usable.length ? usable.filter(row => row.json_valid).length / usable.length : null,
...
const outputPath = path.join(RESULTS_DIR, `${stamp}-locomo-ollama-oracle-v2-${modelSlug}.json`);
await writeFile(outputPath, `${JSON.stringify({
  generated_at: new Date().toISOString(),
  benchmark: "locomo-generic-ollama-oracle-v2",
  model,
  endpoint,
  per_category: perCategory,
  enable_thinking: enableThinking,
  answer_max_tokens: enableThinking ? Math.max(answerMaxTokens, 256) : answerMaxTokens,
  summary,
  rows,
}, null, 2)}\n`, "utf8");

console.log("\n=== LOCOMO GENERIC OLLAMA ORACLE v2 SUMMARY ===");
const cats = Object.entries(summary.by_category)
  .map(([name, value]) => `${name}=${value.f1 == null ? "n/a" : `${(value.f1 * 100).toFixed(1)}%`}`)
  .join(" ");
console.log(`ollama    F1=${summary.f1 == null ? "n/a" : `${(summary.f1 * 100).toFixed(1)}%`} errors=${summary.errors} JSON=${summary.json_valid_rate == null ? "n/a" : `${(summary.json_valid_rate * 100).toFixed(1)}%`} ${cats} answer_p50=${summary.answer_latency_ms.p50}ms p95=${summary.answer_latency_ms.p95}ms mean_tps=${summary.mean_tokens_per_s == null ? "n/a" : summary.mean_tokens_per_s.toFixed(1)}`);
