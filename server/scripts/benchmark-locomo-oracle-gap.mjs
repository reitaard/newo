...
const answerMaxTokens = Number(process.env.LOCOMO_ANSWER_MAX_TOKENS ?? 80);
...
const MINIMAL_EXTRACTOR = `You compress conversation evidence into durable memory facts for later question answering. Return exactly one JSON object with top-level "memory" array; each item contains only "text". Capture only facts actually asserted in the evidence. Make every memory self-contained: resolve pronouns to named entities when the evidence permits it. Preserve names, dates, numbers, relationships, locations, preferences, meaningful events, and explicit plans. Resolve relative dates using the supplied session timestamp when possible. Ignore greetings, questions, conversational filler, and unsupported inference. Split independent facts into separate memories. Empty memory is valid.`;

const VOICEMEM_EXTRACTOR = `You are an additive long-term memory extractor for conversation evidence. Return exactly one JSON object with top-level "memory" array; each item contains only "text". Store durable user/world knowledge that remains useful in another conversation. Memories must be atomic and self-contained, resolving pronouns to named entities when possible. Preserve proper nouns, dates, numbers, relationships, locations, significant events, and explicit plans; resolve relative dates from the supplied session timestamp when possible. Do not store greetings, questions, one-off requests, conversation-meta remarks, assistant-like suggestions, uncertain speculation, or statements whose only content is that something was asked or said. Do not invent facts. Empty memory is valid and preferred over junk.`;

const ANSWER_SYSTEM = `Answer the question using only the supplied evidence. Return only the shortest answer that fully answers the question, with no explanation or preamble. Preserve names, dates, numbers, and list items. If the evidence is insufficient, answer exactly: Not mentioned.`;
...
function selectQuestions(dataset) {
  ...
  for (const category of categoryIds) {
    ...
    for (let sampleIndex = 0; sampleIndex < dataset.length && categoryRows.length < perCategory; sampleIndex++) {
      const sample = dataset[sampleIndex];
      const qaIndex = (sample.qa ?? []).findIndex((qa, index) => {
        if (Number(qa.category) !== category || used.has(`${sampleIndex}:${index}`)) return false;
        return evidenceForQa(sample, qa).complete;
      });
      if (qaIndex < 0) continue;
      const qa = sample.qa[qaIndex];
      const evidence = evidenceForQa(sample, qa);
      const id = `${sample.sample_id ?? `sample${sampleIndex + 1}`}:q${qaIndex + 1}`;
      categoryRows.push({ id, sampleIndex, qaIndex, sample, qa, evidence });
      used.add(`${sampleIndex}:${qaIndex}`);
    }

    if (categoryRows.length < perCategory) {
      outer: for (let sampleIndex = 0; sampleIndex < dataset.length; sampleIndex++) {
        const sample = dataset[sampleIndex];
        for (let qaIndex = 0; qaIndex < (sample.qa ?? []).length; qaIndex++) {
          if (categoryRows.length >= perCategory) break outer;
          if (used.has(`${sampleIndex}:${qaIndex}`)) continue;
          const qa = sample.qa[qaIndex];
          if (Number(qa.category) !== category) continue;
          const evidence = evidenceForQa(sample, qa);
...
function formatEvidence(turns) {
  return turns.map(turn => {
    const date = turn.session_date ? ` | ${turn.session_date}` : "";
...
async function extractMemories(style, evidenceText) {
  ...
  const parsed = extractJson(generated.raw);
  if (!Array.isArray(parsed?.memory)) throw new Error('extractor response missing "memory" array');
...
async function answerQuestion(contextLabel, contextText, question) {
  const generated = await requestGenerate({
    model,
    prompt: fastPrompt(ANSWER_SYSTEM, `${contextLabel}:\n${contextText}\n\nQuestion: ${question}`),
    raw: true,
    stream: true,
    keep_alive: -1,
    options: {
  ...
  const answer = generated.raw.replace(/<think>[\s\S]*?<\/think>/gi, "").trim();
  return { answer, metrics: generated.metrics };
}
...
function aggregateLane(rows, lane) {
  ...
  const scores = usable.map(row => row.lanes[lane].score);
...
console.log(`\nLoCoMo Oracle-Gap: model=${model} endpoint=${endpoint}`);
...
if (summary.oracle) {
  for (const lane of ["minimal", "voicemem"]) {
    ...
    summary[lane].by_category_gap_vs_oracle = Object.fromEntries(
      Object.entries(summary[lane].by_category).map(([name, value]) => [
        name,
        (summary.oracle.by_category[name]?.f1 == null || value.f1 == null)
          ? null : summary.oracle.by_category[name].f1 - value.f1,
      ]),
    );
  }
}
...
const outputPath = path.join(RESULTS_DIR, `${stamp}-locomo-oracle-gap-${modelSlug}.json`);
const report = {
  generated_at: new Date().toISOString(),
  benchmark: "locomo-oracle-gap-30",
  ...
  scoring_note: "LoCoMo-style lexical F1 normalization in JS without Porter stemming; intended for within-run oracle-gap/model comparison, not official leaderboard reporting.",
  summary,
  rows,
};
await writeFile(outputPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");

console.log("\n=== LOCOMO ORACLE-GAP SUMMARY ===");
for (const lane of ["oracle", "minimal", "voicemem"]) {
  const s = summary[lane];
  if (!s) continue;
  const gap = s.compression_gap_vs_oracle == null ? "" : ` gap=${(s.compression_gap_vs_oracle * 100).toFixed(1)}pp`;
  const cats = Object.entries(s.by_category).map(([name, value]) => `${name}=${value.f1 == null ? "n/a" : `${(value.f1 * 100).toFixed(1)}%`}`).join(" ");
  console.log(`${lane.padEnd(9)} F1=${s.f1 == null ? "n/a" : `${(s.f1 * 100).toFixed(1)}%`}${gap} ${cats} answer_p50=${s.answer_latency_ms.p50}ms extract_p50=${s.extract_latency_ms.p50 ?? "-"}ms`);
}
console.log(`\nSaved ${outputPath}`);
