...
const baseUrl = String(process.env.LOCOMO_QWEN_BASE_URL ?? "http://127.0.0.1:8181").replace(/\/+$/, "");
const endpoint = process.env.LOCOMO_QWEN_URL ?? `${baseUrl}/v1/chat/completions`;
const model = process.env.LOCOMO_QWEN_MODEL ?? "helix-qwen3-0.6b";
const apiKey = process.env.LOCOMO_QWEN_API_KEY ?? "";
...
function parseAnswer(content) {
  ...
  if (start >= 0 && end > start) {
    try {
      ...
      if (typeof parsed.answer === "string") return { answer: parsed.answer.trim(), json_valid: true };
    } catch {}
  }
...
async function answerQuestion(evidenceText, question) {
  ...
  if (typeof content !== "string") throw new Error("OpenAI-compatible response missing choices[0].message.content");
  ...
  return {
    ...
    metrics: {
      ...
      prompt_tokens: Number.isFinite(payload?.usage?.prompt_tokens) ? payload.usage.prompt_tokens : null,
      output_tokens: Number.isFinite(payload?.usage?.completion_tokens) ? payload.usage.completion_tokens : null,
    },
  };
}
...
console.log(`\nLoCoMo VPS Qwen oracle control: model=${model} endpoint=${endpoint}`);
...
console.log("This is a lower-bound model control: Qwen3 0.6B is much smaller than LFM2.5-8B.\n");
...
for (let i = 0; i < selected.length; i++) {
  ...
  try {
    ...
    console.log(`  qwen      F1=${(row.score * 100).toFixed(1)}% answer=${JSON.stringify(row.prediction)} json=${row.json_valid}`);
  } catch (error) {
    row.error = error.message;
    row.score = null;
    console.log(`  qwen      ERR ${error.message}`);
  }
  rows.push(row);
}
...
const outputPath = path.join(RESULTS_DIR, `${stamp}-locomo-qwen-oracle-${model.replace(/[^a-z0-9._-]+/gi, "-")}.json`);
await writeFile(outputPath, `${JSON.stringify({
  generated_at: new Date().toISOString(),
  benchmark: "locomo-qwen-oracle-control",
  model,
  endpoint,
...
console.log("\n=== LOCOMO VPS QWEN ORACLE SUMMARY ===");
const cats = Object.entries(summary.by_category)
  .map(([name, value]) => `${name}=${value.f1 == null ? "n/a" : `${(value.f1 * 100).toFixed(1)}%`}`)
  .join(" ");
console.log(`qwen      F1=${summary.f1 == null ? "n/a" : `${(summary.f1 * 100).toFixed(1)}%`} errors=${summary.errors} JSON=${summary.json_valid_rate == null ? "n/a" : `${(summary.json_valid_rate * 100).toFixed(1)}%`} ${cats} answer_p50=${summary.answer_latency_ms.p50}ms p95=${summary.answer_latency_ms.p95}ms`);
