...
The existing Qwen3 llama.cpp service is OpenAI-chat compatible and must remain private. It remains the default provider. Configure its private URL and model alias, for example `ASSISTANT_ENABLED=true`, `ASSISTANT_PROVIDER=openai_chat`, `ASSISTANT_BASE_URL=http://127.0.0.1:8181`, and `ASSISTANT_MODEL=helix-qwen3-0.6b`. `ASSISTANT_TIMEOUT_MS`, `ASSISTANT_MAX_OUTPUT_TOKENS`, and `ASSISTANT_MAX_REPLY_CHARS` bound request time and spoken output. No API key is needed for the local service unless its deployment adds one.

For a direct remote Ollama baseline, retain the same bounds and set `ASSISTANT_PROVIDER=ollama_raw`, `ASSISTANT_BASE_URL=http://100.68.131.86:11435`, and `ASSISTANT_MODEL=newo-main`. This calls Ollama's streaming `/api/generate` endpoint directly; it does not proxy through another service or stream partial text to TTS.
