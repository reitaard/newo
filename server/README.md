...
A finalized Sherpa transcript can make one bounded assistant request and, when it returns non-empty text, uses the existing Pocket-to-Opus `/speaker` path. Partials and cleanup finals never invoke the model, and one active assistant turn is allowed per device. The assistant retains only selectively requested bounded history and does not create another audio transport. A dormant tool boundary parses native LFM envelopes, validates schemas, and enforces host authorization; no tools are registered until an explicit production tool set is supplied.
...
The built-in `qwen3:0.6b` (`qwen`) profile preserves the private OpenAI-chat-compatible Qwen service and remains the default. The built-in `lfm2.5:8b` (`lfm`) profile owns the direct Ollama request, native reasoning prompt, incremental `<think>` filtering, sampling, timeout, and response bounds. Hidden reasoning is generated natively but never enters progressive TTS or the final spoken answer. The deterministic router selects FAST for low-risk single-step requests and THINK when request, context, or available-tool structure signals uncertainty, consequences, or planning. That decision is interpreted by the effective profile: LFM FAST uses its closed-think bridge and LFM THINK uses native reasoning, while both Qwen routes retain Qwen's OpenAI template and model-default reasoning behavior. Set `ASSISTANT_PROFILE=qwen` or `ASSISTANT_PROFILE=lfm` for the initial preference; Telegram `/profile_qwen`, `/profile_lfm`, and `/profile qwen|lfm` switch and persist it at runtime. `/profile` reports both the preferred and currently effective profile. Short aliases are `/p`, `/p_lfm`, and `/p_qwen`; `/ping` and `/pi` run the latency check. Hidden tuning controls are `/p_conf`, `/p_reset`, and `/p set|s <key> <value>`, where the bounded keys are `topk`, `topp`, `maxtoken`, `maxchars`, `timeout`, `rpenalty`, and `temp`. `/p s sysprompt` starts a two-step edit of the preferred profile's system prompt; the next plain-text message is saved, while `/cancel` exits without changing it. `/profile_tune` (alias `/pt`) also shows or applies the `fast`, `balanced`, `quality`, and `reset` persistent presets. Tuning never exposes or changes endpoints, API keys, providers, models, or fallback wiring in Telegram.

### Xiaomei bilingual runtime

Set `XIAOMEI_ENABLED=true` after the model and speech services are ready. Telegram `/translate`, `/translate on`, `/translate off`, and `/translate status` control the persisted Alfred/Xiaomei GPU-residency mode. Xiaomei owns voice transcripts while that mode is active; saying `玲玲`, `Ling Ling`, `Xiaomei`, or `小美` also resets the current session to chat. Clear state-aware commands run deterministically; ordinary active-interpreter speech goes directly through `newo-translate:latest`; chat, teaching, and interpreter-meta explanations use `newo-gemma-e2b:latest`; spoken Xiaomei output selects the Qwen3-TTS backend with the `Serena` speaker without changing Alfred's Pocket/Kokoro/eSpeak backend.

Configure `XIAOMEI_OLLAMA_BASE_URL` for the shared OpenAI-compatible Gemma/Hy-MT2 endpoint and `XIAOMEI_TTS_BASE_URL` for Serena plus `/admin/load` and `/admin/unload`. Separate `XIAOMEI_GEMMA_BASE_URL` and `XIAOMEI_TRANSLATE_BASE_URL` overrides remain available. The TTS endpoint must return raw mono 24 kHz PCM16 from `POST /v1/audio/speech`. Exact pronunciation remains behind a provider boundary and is intentionally reported as unverified when no trusted provider exists. Run `npm run xiaomei:diagnose` for an offline text-only route/state simulation; it does not claim live model, GPU, TTS, or ESP validation.
...
`src/voice.js` defines the replaceable streaming ASR boundary: `createStream({ format, onEvent })`, `acceptAudio(chunk)`, and `stop()`. With `VOICE_ASR_BACKEND=sherpa`, `src/sherpa-worker.js` exclusively loads the native addon. With `VOICE_ASR_BACKEND=sherpa-python`, one persistent localhost Python process loads the official `sherpa-onnx` recognizer once and uses the same bounded PCM/event boundary. Startup primes the selected recognizer before accepting connections. If Python or its LM fails during startup, Newo terminates that process before lazily loading the existing Node worker; both recognizers are never intentionally resident together. `VOICE_ASR_BACKEND=null` retains the transport-only fallback.
...
Use `VOICE_SHERPA_MODEL=20m npm run voice:benchmark` for the 20M comparison and `VOICE_SHERPA_MODEL=libri-giga npm run voice:benchmark` for the default large-model benchmark. Add `VOICE_ASR_BACKEND=sherpa-python` to benchmark the Python process; the script uses the same WAV, PCM chunking, endpoint rules, model files, and LM manifest checks.
