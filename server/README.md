...
`/voice` (alias `/v`) is the current manual one-turn trigger: it starts OFF → STREAMING directly, and a second use cancels to OFF. `/speaker` and `/eco` remain pure toggles. Each waits for its applicable acknowledgement and returns a detailed subsystem status. The existing `/vs` alias remains a read-only detailed voice status and labels the trigger as manual with wake word deferred. Speaker controls are `/volume`, `/volume <0-100>`, `/mute`, and `/speak <text>` (alias `/sp`; `/s` remains the established status alias). Existing operational aliases include `/s`, `/h`, `/l`, `/e`, `/r`, and `/f`; `/p` now reports the assistant profile and `/pi` is the short ping alias. `/errors` behavior is unchanged.
