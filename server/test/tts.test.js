...
test("delivery-aware flow separates network flight, ESP buffer, and total outstanding", () => {
  ...
  assert.equal(speakerCreditBytes(0, 0, 0, 0), 8_192, "initial credit is bounded by the separate network flight window");
  assert.equal(speakerCreditBytes(8_192, 0, 0, 0), 0, "sent callbacks do not count as ESP delivery");
  assert.equal(speakerCreditBytes(14_336, 8_192, 0, 8_192), 0, "delivered plus in-flight PCM must not exceed the committed receiver target");
  assert.equal(speakerCreditBytes(14_336, 14_336, 2_048, 12_288), 2_048, "consumption replenishes exactly one frame of receiver credit");
...
test("delivery credit waits for actual ESP receipt and replenishes its reported buffer", async () => {
  ...
  await tick();
  assert.equal(binaryFrames(ws).length, 4, "sender must stop at the separate 8 KiB network-flight window before receipt");
  assert.equal(ws.frames.some((frame) => String(frame).includes("speaker_end")), false);
