...
test("Phnom Penh current time and full date survive the UTC date boundary", () => {
  ...
  const timeRequest = parseClockRequest("what time is it", localOptions);
  assert.deepEqual(timeRequest, { kind: "local", action: "current_time" });
  assert.equal(
    formatClockReply(timeRequest, { applied: true }, localOptions),
    "It is 1:30 AM."
  );

  const dateRequest = parseClockRequest("what's today's date", localOptions);
  assert.deepEqual(dateRequest, { kind: "local", action: "current_date" });
  assert.equal(
    formatClockReply(dateRequest, { applied: true }, localOptions),
    "Today is Tuesday, September 22, 2026."
  );
});

test("alarm epochs are computed from Phnom Penh wall time rather than UTC wall time", () => {
  const boundaryNow = new Date("2026-09-21T18:30:00Z");
  const request = parseClockRequest("set an alarm for 7 AM tomorrow", {
    now: boundaryNow,
    timeZone: "Asia/Phnom_Penh",
  });
  ...
  // Local date is already September 22 at this instant.
  // Tomorrow 07:00 ICT is September 23 00:00 UTC.
  assert.equal(
