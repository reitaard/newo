...
def train_model(args, labels, train_rows, eval_rows):
    ...
    print(
        f"trained rows={len(filtered_rows)} classes={len(labels)} device={device} "
        f"base={args.base_model} load_ms={model_load_ms:.1f} train_ms={training_ms:.1f} "
        f"removed_exact_eval_leaks={len(leakage)} counts={dict(train_counts)}",
        file=sys.stderr,
    )
    if leakage:
        print(
...
def benchmark_model(args, labels, registry, eval_rows):
    ...
    for row in eval_rows:
        ...
        ranked_idx = np.argsort(-vector)
        best = int(ranked_idx[0])
        primary_label = labels[best]
        confidence = float(vector[best])
        margin = float(vector[best] - vector[int(ranked_idx[1])]) if len(ranked_idx) > 1 else confidence
        scores = {label: float(vector[index]) for index, label in enumerate(labels)}

        if primary_label == ABSTAIN_LABEL:
            primary = None
            capabilities = []
            abstain = True
            need = "unknown"
            ...
            capabilities = [primary_label]
            abstain = False
            need = source_need(primary_label, registry)

        output_rows.append({
            "id": row["id"],
            "primary": primary,
            "raw_primary": primary_label,
...
def main():
    parser = argparse.ArgumentParser(description="Train and benchmark conservative SetFit capability router v2")
    parser.add_argument("corpus", nargs="?", default="corpus.jsonl")
    parser.add_argument("output", nargs="?", default="setfit-v2-predictions.jsonl")
    ...
    parser.add_argument("--model-dir", default="setfit-minilm-router-v2")
