...
async function appendXiaomeiCommandToTelegramMenu() {
  ...
  try {
    ...
    if (!current?.ok || !Array.isArray(current.result) || current.result.some((item) => item?.command === "xiaomei")) return;
    const commands = [...current.result, { command: "xiaomei", description: "Toggle Alfred / Xiaomei" }];
...
export function createPrimaryModeHandlers(options) {
  ...
  if (enabled) {
    ...
    const menuTimer = setTimeout(() => { void appendXiaomeiCommandToTelegramMenu(); }, 3_000);
    menuTimer.unref?.();
  }
  ...
  async function xiaomei(ctx, forced = null) {
  ...
  async function profilePromptInput(ctx) {
    ...
    const command = raw.match(/^\/(?:xiaomei|xm)(?:@[a-z0-9_]+)?(?:\s+(.+))?$/i);
    if (command) {
      await xiaomei(ctx, command[1] ?? "");
      return true;
    }
    return core.profilePromptInput(ctx);
  }

  return { ...core, xiaomei, profilePromptInput, xiaomeiStatus: modeSwitch.status };
}
