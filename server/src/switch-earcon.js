...
export function wrapTtsBackendWithSwitchEarcons(backend) {
  // Preserve the selected backend's public type contract while overriding only
  // synthesis entry points. Existing callers use instanceof for configuration
  // diagnostics and should still see Pocket/Kokoro/eSpeak, not an anonymous shim.
  const wrapped = Object.assign(Object.create(Object.getPrototypeOf(backend)), {
    name: backend.name, voice: backend.voice, gainDb: backend.gainDb, limiter: backend.limiter,
  });
