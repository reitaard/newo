...
import { createAssistantProfiles, GEMMA_PROFILE_ID, LFM_PROFILE_ID, MINICPM_PROFILE_ID, MINISTRAL_PROFILE_ID, SPARK_PROFILE_ID, normalizeProfileTuning, PROFILE_TUNING_PRESETS, QWEN_PROFILE_ID, resolveAssistantProfile } from "./assistant-profiles.js";
import { createAssistantTurnRuntime } from "./assistant-turn.js";
import { createClockTranscriptHandler } from "./clock-transcript.js";
import { createClockSemanticResolver } from "./clock-semantic.js";
...
const EnvSchema = z.object({
  ...
  CAPABILITY_ROUTER_ENABLED: z.preprocess(stringToBoolean, z.boolean().default(false)),
...
const clockSemanticResolve = createClockSemanticResolver({
  profile: assistantProfiles[MINICPM_PROFILE_ID],
  logger: app.log,
});
...
const assistantWebTools = createAssistantWebTools({
  enabled: env.ASSISTANT_WEB_TOOLS_ENABLED,
  baseUrl: env.AGENT_TOOLS_BASE_URL,
  token: readAgentToolsToken({ envFile: env.AGENT_TOOLS_TOKEN_FILE }),
  timeoutMs: env.AGENT_TOOLS_TIMEOUT_MS,
  logger: app.log,
});
const capabilityRouter = createCapabilityRouterClient({
  enabled: env.CAPABILITY_ROUTER_ENABLED,
  baseUrl: env.CAPABILITY_ROUTER_BASE_URL,
...
const handleClockTranscript = createClockTranscriptHandler({
  ...
  sendAssistantState,
  semanticResolve: clockSemanticResolve,
  logger: app.log,
});
...
const DeviceMessageSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("clock_command_ack"),
    request_id: z.string(),
    applied: z.boolean(),
...
const primaryModeHandlers = createPrimaryModeHandlers({
  ...
  setAssistantTuningPreset: async (preset) => {
    ...
    if (preset === "reset" || preset === "balanced") delete nextOverrides[id];
      ...
      nextOverrides[id] = (id === GEMMA_PROFILE_ID || id === MINICPM_PROFILE_ID || id === SPARK_PROFILE_ID)
  ...
  setAssistantTuningValue: async (key, value) => {
    ...
    if (canonical === "think_mode" && id !== GEMMA_PROFILE_ID && id !== MINICPM_PROFILE_ID && id !== SPARK_PROFILE_ID) throw new Error("thinking control is not available for this profile");
...
if (env.TELEGRAM_BOT_TOKEN) {
  ...
  bot.command(["profile_minicpm", "p_minicpm"], (ctx) => primaryModeHandlers.profile(ctx, "minicpm"));
  bot.command(["profile_ministral", "p_ministral"], (ctx) => primaryModeHandlers.profile(ctx, "ministral"));
  bot.command(["profile_spark", "p_spark"], (ctx) => primaryModeHandlers.profile(ctx, "spark"));
