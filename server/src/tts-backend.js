...
import { wrapTtsBackendWithSwitchEarcons } from "./switch-earcon.js";
...
export function createTtsBackend(env, logger) {
  ...
  let backend;
  if (env.TTS_BACKEND === "pocket") backend = new PocketTtsBackend({
  ...
  else if (env.TTS_BACKEND === "kokoro") backend = new KokoroTtsBackend({
  ...
  else backend = new EspeakTtsBackend({ voice, rate: env.TTS_RATE, gainDb, maxPcmBytes: env.TTS_MAX_PCM_BYTES });
  return wrapTtsBackendWithSwitchEarcons(backend);
}
