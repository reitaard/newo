...
Only the autonomous neutral IDLE face receives Autonomy V2. The 20 FPS display loop reuses Phase A's bounded gaze interpolation, with readable side and upper targets, varied fixation, and occasional small correction. Phase B's bilateral blink scheduler remains the sole owner of normal, double, long, and post-saccade blinks; manual one-eye wink styles remain separate. Four volatile `uint8_t` signals—energy, curiosity, social, and stress—update every three seconds. They drive four bounded episodes (`CURIOUS_SCAN`, `LOW_ENERGY`, `SOCIAL_ATTENTION`, and `ALERT_CHECK`) spaced 10–20 seconds apart; episodes request the existing blink scheduler and use no task, framebuffer, library, persistence, or cloud control. ACTIVE, RELAXED, and DROWSY stages bias timing and episode selection after two and five minutes of inactivity. `/face_default` enables autonomy and all other face styles are manual overrides. Effective display context is arbitrated as special screen, ERROR, LISTENING, SPEAKING, THINKING, then IDLE, with runtime signals independent of persistent Telegram mode commands. USB Serial reports context, gaze, episode, and blink events, with cumulative `EYES_STATS` rate-limited to once per minute.
