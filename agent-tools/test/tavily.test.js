...
test("Tavily search adapter disables provider answers and maps evidence", async () => {
  ...
  const provider = new TavilyProvider({ apiKey: "hidden", fetchImpl: async (url, options) => {
    ...
    return new Response(JSON.stringify({ response_time: 0.123, results: [{ title: "A", url: "https://a.test", content: "summary", raw_content: "full", score: 0.7, published_date: "2026-01-02" }] }), { status: 200 });
  } });
  const outcome = await provider.search({ query: "q", topic: "general", depth: "basic", maxResults: 2, includeDomains: [], excludeDomains: [], includeContent: true }, new AbortController().signal);
  ...
  assert.equal(outcome.results[0].content, "full");
  assert.equal(outcome.providerElapsedMs, 123);
});
