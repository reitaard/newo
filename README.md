...
`https://smonitor.reitaard.de`; its browser stream automatically uses
`wss://smonitor.reitaard.de/smonitor/ws`. The Tailscale MagicDNS fallback is
...
Voice defaults **ARMED** with local WakeNet. **Hi Wall-E** opens the same authenticated `/voice` stream and re-arms after the turn; `/v` remains a manual one-shot that settles OFF. WakeNet releases microphone I2S before streaming and remains suppressed throughout speaker playback. The direct session sends 20 ms 16 kHz mono PCM16 frames to Sherpa, then uses the selected assistant profile, Pocket, and existing Opus speaker path. There is no second PCM queue or cloud sender task.

The written product identity is **Newo** and the spoken assistant name is **Neo**. The local trigger is Espressif `wn9_hiwalle_tts2`; the matching custom `srmodels.bin` must be generated and uploaded because Arduino-ESP32's stock package currently contains Hi ESP. Sherpa's `NEO` hotword remains transcription bias only. See [`docs/wake-word.md`](docs/wake-word.md).
...
Open `Newo/Newo.ino`. Build with Arduino-ESP32 3.3.10 and `esp32:esp32:esp32s3:UploadSpeed=921600,USBMode=hwcdc,CDCOnBoot=default,MSCOnBoot=default,DFUOnBoot=default,UploadMode=default,CPUFreq=240,FlashMode=qio,FlashSize=16M,PartitionScheme=esp_sr_16,DebugLevel=none,PSRAM=opi,LoopCore=1,EventsCore=1,EraseFlash=none,JTAGAdapter=default,ZigbeeMode=default`. The ESP-SR model partition is mandatory. Because Arduino's packaged image currently selects Hi ESP, compile first, run `Newo/prepare_hiwalle_srmodels.ps1` to replace that output directory's model with `wn9_hiwalle_tts2`, and upload the same output directory without recompiling. Never use `compile --upload` after model preparation. Exact COM7-only commands are in [`docs/wake-word.md`](docs/wake-word.md). Do not erase flash. Hidden Telegram `/owner_enroll`, `/owner_status`, and `/owner_cancel` manage the separate server-side voiceprint; `/v` remains the manual microphone one-shot.
