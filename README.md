...
The default neutral IDLE face uses a lightweight, runtime-only autonomous system. Phase A selects a center-biased gaze target, moves with a quick settling saccade, holds it for a varied fixation, and may make one tiny correction. Phase B schedules bilateral blinks with 2.5–6 second calm intervals, rare double blinks, rare longer blinks, and occasional post-saccade blinks; `wink_left` and `wink_right` retain their independent one-eye bursts. Phase C adds four volatile `uint8_t` values—energy (70), curiosity (42), social (38), and stress (5)—that drift every three seconds and respond only to existing display state transitions and changed face selections. They subtly affect neutral-IDLE gaze weighting, fixation time, and blink timing without selecting expressions or changing fixed look styles, special faces, or operational modes. Phase D separately evaluates one neutral-IDLE micro-behavior every 4–9 seconds; most evaluations stay still, while rare glances, existing-scheduler double/long blinks, brief lift/squint geometry, a single wink, or a 500–1200 ms rest-close are personality and inactivity weighted. Inactivity is runtime-only: ACTIVE is under two minutes, RELAXED is two to five minutes, and DROWSY is over five minutes. The state is never persisted and emits one concise USB-Serial `EYES_STATE` line at most once per minute.
