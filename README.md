...
Audio uses a dedicated FreeRTOS capture task and I2S DMA, feeding a 24-frame (480 ms) bounded queue. It keeps 20 ms / 640-byte PCM frames internally but joins exactly five contiguous frames into each 3,200-byte binary WebSocket message (100 ms, approximately 10 sends/sec). The loop-owned WebSockets client drains at most one complete five-frame bundle per healthy loop; it never sends a partial bundle, padding, framing bytes, or stale data. Queue overflow and reconnect transitions discard stale frames rather than replay speech.
