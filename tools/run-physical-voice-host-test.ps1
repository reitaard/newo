...
$output = Join-Path $repo 'build/physical-voice-host-test.exe'
New-Item -ItemType Directory -Force (Split-Path $output -Parent) | Out-Null
& $compiler -std=c++17 -Wall -Wextra -Werror '-static' (Join-Path $repo 'tools/physical-voice-host-test.cpp') -o $output
