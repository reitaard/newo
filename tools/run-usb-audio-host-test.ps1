...
$output = Join-Path $repo 'build/usb-audio-host-test.exe'
New-Item -ItemType Directory -Force (Split-Path $output -Parent) | Out-Null
& $compiler -std=c++17 -Wall -Wextra -Werror '-static' (Join-Path $repo 'tools/usb-audio-host-test.cpp') (Join-Path $repo 'Newo/newo_usb_audio_descriptors.cpp') -o $output
